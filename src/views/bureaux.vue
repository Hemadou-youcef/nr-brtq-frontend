<template>
  <div class="home">
    ...
    >
      <AddItem :category="'bureaux'" :closeDialog="closeOverLay"/>
    </v-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    GetAllfurniture() {
      this.loading = true;
      this.axios.get("/items?category=bureaux").then((response) => {
        this.loading = false;
    ...
    pagination() {
      ...
      if (this.search == "") {
        this.axios
          .get("/items?category=bureaux&page=" + this.page)
          .then((response) => {
            this.loading = false;
            this.furniture = response.data.data;
        ...
        this.axios
          .get("/items/" + this.search + "?category=bureaux&?page=" + this.page)
          .then((response) => {
            this.loading = false;
            this.furniture = response.data.data;
            this.pageLength = response.data.meta.last_page;
